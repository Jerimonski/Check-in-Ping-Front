import GeneralStatusCards from "./ui/GeneralStatusCards"
import { CiWarning } from "react-icons/ci"
import { MdOnlinePrediction } from "react-icons/md"
import { ImConnection } from "react-icons/im"
import { LuSearchX } from "react-icons/lu"
import usePing from "../hooks/usePing"

export default function StatusSection() {
  const { upCount, downCount, lostCount, totalCount } = usePing()

  return (
    <div className="flex flex-grow justify-between gap-8 py-10">
      <GeneralStatusCards
        BorderColor={"border-green-500"}
        TextColor={"text-green-500"}
        Title={"Activos"}
        ValueInNetwork={upCount}
        Icon={<ImConnection size={45} color="LimeGreen" />}
      />
      <GeneralStatusCards
        BorderColor={"border-red-600"}
        TextColor={"text-red-500"}
        Title={"Caidos"}
        ValueInNetwork={downCount}
        Icon={<CiWarning size={45} color="red" />}
      />
      <GeneralStatusCards
        BorderColor={"border-yellow-400"}
        TextColor={"text-yellow-700"}
        Title={"Perdido/s"}
        ValueInNetwork={lostCount}
        Icon={<LuSearchX size={45} color="darkkhaki" />}
      />
      <GeneralStatusCards
        BorderColor={"border-blue-400"}
        TextColor={"text-blue-500"}
        Title={"total IP's"}
        ValueInNetwork={totalCount}
        Icon={<MdOnlinePrediction size={45} color="DodgerBlue" />}
      />
    </div>
  )
}
